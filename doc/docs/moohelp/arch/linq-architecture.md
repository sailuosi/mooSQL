...
| **定位** | mooSQL 自研特色查询层，贴近项目实践 | 对标 EF Core、SqlSugar `Queryable` 等通用 LINQ 能力 |
| **入口** | **`useBus` / `useDbBus`** → `IDbBus<T>` / `EnDbBus<T>` | **`useQueryable` / `AsQueryable`** → `IDbQuery<T>` |
| **工厂** | `FastLinqFactory` → `FastLinqCompiler` | `EntityLinqFactory` / `EntityVisitFactory` → `EntityQueryCompiler` / `EntityVisitCompiler` |
| **扩展 API** | `BusQueryable`：`Set`、`DoUpdate`、`LeftJoin`、`ToPageList`、`InjectSQL` 等 moo 扩展 | 标准 `System.Linq.Queryable` + mooSQL 扩展（Includes、Merge、SetOp、`DbFunc` 等） |
...
var table = db.useQueryable<User>();       // 或 db.AsQueryable<User>() / DBCash.useQueryable<User>(0)
...
**Ext LINQ 去 Linq2DB 化（2026-06-06）**：Phase A–E 已完成（**162/162** TestLinq net6.0）；Phase G 收尾注释/`ExtLinqOptions`；Phase F — Collate registry、Over IR（[`ADR-PhaseF-AnalyticOver-IR.md`](../../../../ext/src/linq/core/ADR-PhaseF-AnalyticOver-IR.md)）、StringAgg 延期（[`ADR-PhaseF-StringAggregate-Deferral.md`](../../../../ext/src/linq/core/ADR-PhaseF-StringAggregate-Deferral.md)）；E.5 多语句事务、E.6 `StreamQueryAsync` 逐行读库已落地。
