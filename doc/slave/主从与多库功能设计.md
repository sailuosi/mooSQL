...
| API | 状态 |
|-----|------|
| `DBInsCash.GetRead` / `GetWrite` | **删除**（无历史包袱） |
| `DBInsCash.resolveRead` / `resolveWrite`（public） | **internal 迁至 MooClient/RouteResolver** |
| `SQLBuilder.ResolveRouteBeforeExecute` + 执行前 `setDBInstance` | **删除**，改 Executor 内解析 |
