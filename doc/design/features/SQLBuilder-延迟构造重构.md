...
| D7 | 门面形态 | 门面 **组合** `StepBuilder _inner`（不继承）；构造 API 入队，出口 `EnsureMaterialized` 后委托内核；`IStep.Apply(SQLBuilder)`，步骤内写 `Inner` 防重入入队；嵌套 `Action<SQLBuilder>` 仅在 brother/Kit 侧 `Attach` 一次 | **已落地** |
...
| D13 | 延迟参数解析（IDelayPara） | 动态片段 PlaceHolder + 构建期 `Run()`；一期 `whereInGuid` / `whereFormat`；见 [SQLBuilder-延迟参数解析](./SQLBuilder-延迟参数解析.md) | 规划中 |
