...
namespace mooSQL.excel.context
{
    ...
    public class WriteTable// :ImportOption.Table
    {
        ...
        /// <summary>批量更新模式下，当前正在组装的单行 <see cref="SQLBuilder"/>。</summary>
        public SQLBuilder rowKit;
        /// <summary>当前行已通过 <see cref="set(string, object)"/> 赋值的字段个数。</summary>
        public int setCount = 0;
        /// <summary>
        /// 向当前批量更新行写入一列键值；若尚未开始行，会自动调用 <see cref="StartRow"/>。
        /// </summary>
        ...
        /// <summary>开始组装新的一行批量更新数据（重置计数并申请行构造器）。</summary>
        ...
        /// <summary>若当前行已有字段赋值，则提交为一条 UPDATE 并结束行状态。</summary>
        ...
        /// <summary>行级更新时缓存的列名到字符串值的映射（如中间态拼接 WHERE/SET）。</summary>
