...
namespace mooSQL.excel
{
    ...
    public class InField
    {
        ...
        */
        ...
        /// <summary>加载失败时的提示文案。</summary>
        public string loadErrTip;

        /// <summary>预匹配使用的正则表达式。</summary>
        public string preMatchReg;

        /// <summary>预匹配失败时的提示信息。</summary>
        public string preMatchMsg;

        /// <summary>固定单元格坐标（如 A1 形式，依实现而定）。</summary>
        public string cell;

        /// <summary>值替换所用的正则。</summary>
        public string replaceReg;

        /// <summary>替换为目标字符串。</summary>
        public string replaceAs;
    }


    /// <summary>
    /// 下拉或码表中的单个选项（显示文本与存储值）。
    /// </summary>
