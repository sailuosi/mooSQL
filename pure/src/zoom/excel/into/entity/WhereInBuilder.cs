...
{
    /// <summary>
    /// 为核验 SQL 构建 IN 条件：收集来源列键（<see cref="srcFields"/>）与字面量值，并继承 <see cref="WhereListBag"/> 的值分类与拼接能力。
    /// </summary>
    public class WhereInBuilder:WhereListBag
    {
        /// <summary>
        /// 参与汇聚 IN 列表值的来源列标识（通常为列配置的 key）。
        /// </summary>
        public HashSet<string> srcFields = new HashSet<string>();
