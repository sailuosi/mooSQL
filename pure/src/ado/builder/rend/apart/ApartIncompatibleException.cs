...
{
    /// <summary>
    /// 碎片与目标 StepBuilder 数据库类型不兼容时抛出（一阶段仅允许同类数据库复用）。
    /// </summary>
    public class ApartIncompatibleException : Exception
    {
        ...
        public ApartIncompatibleException(DataBaseType source, DataBaseType target)
            : base($"SQLApart was captured for {source} but target StepBuilder uses {target}. Cross-database apart reuse is not supported in this version.")
        {
            SourceDbType = source;
            TargetDbType = target;
        }
