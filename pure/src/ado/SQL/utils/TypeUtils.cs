...
#pragma warning restore IL3050
...
#if NETFRAMEWORK
            return GetNullableConstructor(nullableType);
#endif
#if NET6_0_OR_GREATER
        ...
        public static bool HasIdentityPrimitiveOrNullableConversionTo(this Type source, Type dest)
        {
            ...
            if (IsNullableType(source) && AreEquivalent(dest, source.UnwrapNullable()))
            {
                return true;
            }

            if (IsNullableType(dest) && AreEquivalent(source, dest.UnwrapNullable()))
            {
                return true;
            }
            ...
            return IsConvertible(source) && IsConvertible(dest)
                   && (dest.UnwrapNullable() != typeof(bool)
        ...
        public static bool HasReferenceConversionTo(this Type source, Type dest)
        {
            ...
            Type nnSourceType = source.UnwrapNullable();
            Type nnDestType = dest.UnwrapNullable();
        ...
        public static bool HasBuiltInEqualityOperator(Type left, Type right)
        {
            ...
            Type nnType = left.UnwrapNullable();
        ...
        {
            Type nnExprType = convertFrom.UnwrapNullable();
            Type nnConvType = convertToType.UnwrapNullable();
        ...
        private static bool IsImplicitNullableConversion(Type source, Type destination) =>
            IsNullableType(destination) && IsImplicitlyConvertibleTo(source.UnwrapNullable(), destination.UnwrapNullable());
        ...
        public static void ValidateType(Type type, string? paramName, bool allowByRef, bool allowPointer)
        {
            if (ValidateType(type, paramName, -1))
            {
                ...
                {
                    throw new Exception("Error.TypeMustNotBeByRef(paramName)");
                }

                if (!allowPointer && type.IsPointer)
                {
                    throw new Exception("Error.TypeMustNotBePointer(paramName)");
                }
            }
        }
        ...
        public static bool ValidateType(Type type, string? paramName, int index)
        {
            ...
            if (type.ContainsGenericParameters)
            {
                throw type.IsGenericTypeDefinition
                    ? new Exception("Error.TypeIsGeneric(type, paramName, index)")
                    : new Exception("Error.TypeContainsGenericParameters(type, paramName, index)");
            }
        ...
        internal static bool IsUnsigned(this Type type) => IsUnsigned(type.UnwrapNullable().GetTypeCode());
        ...
        internal static bool IsFloatingPoint(this Type type) => IsFloatingPoint(type.UnwrapNullable().GetTypeCode());
