...
namespace mooSQL.data
{
	...
	public abstract class SchemaProviderBase : ISchemaProvider
	{
		...
		public SchemaProviderBase(Dialect parent)
		{
			this.dialect = parent;
		}
		/// <summary>
		...
		public virtual DatabaseSchema GetSchema( GetSchemaOptions? options = null)
		{
			...
			///var dbConnection = DBInstance.Connection;
			...
			foreach (var dt in GetDataTypes())
			...
			if (options.GetTables)
			{
				...
				var pks = GetPrimaryKeys( tables, options);

				#region Columns

				var columns =
					from c  in GetColumns(options)
				...
				var fks = options.GetForeignKeys ? GetForeignKeys( tables, options) : new List<ForeignKeyInfo>();
				...
				var pst = GetProviderSpecificTables( options);
			...
			{
				#region Procedures
				var clauseTranslator = DBLive.dialect.clauseTranslator;
				//var sqlProvider = DBInstance.DataProvider.CreateSqlBuilder(DBInstance.MappingSchema, DBInstance.Options);
				var procs       = GetProcedures(options);
				var n           = 0;

				if (procs != null)
				{
					var procParams = (IEnumerable<ProcedureParameterInfo>?)GetProcedureParameters(procs, options) ?? new List<ProcedureParameterInfo>();
					...
					if (GetProcedureSchemaExecutesProcedure && isActiveTransaction)
						throw new Exception("Cannot read schema with GetSchemaOptions.GetProcedures = true from transaction. Remove transaction or set GetSchemaOptions.GetProcedures to false");
					...
					try
					{
						foreach (var procedure in procedures)
						{
							...
							{
								var commandText = clauseTranslator.TranslateObjectName(
									new (procedure.ProcedureName, Database: procedure.CatalogName, Schema: procedure.SchemaName, Package: procedure.PackageName));

								LoadProcedureTableSchema( options, procedure, commandText, tables);
							}
				...
				var psp = GetProviderSpecificProcedures();
			...
			return ProcessSchema(new DatabaseSchema
			{
				...
				ServerVersion                 = DBLive.config.version,
				Tables                        = tables,
			...
			List<TableSchema> tables)
		{
			...
			try
			{
				var st = GetProcedureSchema( commandText, commandType, parameters, options);
				...
				val     =
					p.SystemType == typeof(string) ?
						...
						p.SystemType == typeof(DateTime) ?
                            ...
                            GetDefaultValue(p.SystemType ?? typeof(object)),
				dataType  = p.DataType,
				//dbType    = p.SchemaType,
				size      = (int?)p.Size,
				direction =
		...
		protected virtual DataTable? GetProcedureSchema( string commandText, CommandType commandType, Parameter[] parameters, GetSchemaOptions options)
		{
			...
			var cmd = new SQLCmd(commandText,ps);
			var res = DBLive.ExeQueryReader(cmd, (rd) =>
			{
		...
		protected virtual List<ColumnSchema> GetProcedureResultColumns(DataTable resultTable, GetSchemaOptions options)
		{
			...
			foreach (DataRow r in resultTable.Rows) {
				...
				var columnType = r.getString("DataTypeName", "");
				var columnName = r.getString("ColumnName","");
				var isNullable = r.getBoolean("AllowDBNull",true);
				var dt = GetDataType(columnType, null, options);
				var length = r.getInt("ColumnSize",0);
				var precision = r.getInt("NumericPrecision",0);
				var scale = r.getInt("NumericScale", 0);
				var systemType = GetSystemType(columnType, null, dt, length, precision, scale, options);


				var tar = new ColumnSchema
				{
					...
					IsIdentity = r.getBoolean("IsIdentity",false),
				};
				columns.Add(tar);
            }
			return columns;
		...
		protected virtual List<DataTypeInfo> GetDataTypes()
		{
			DataTypesSchema = DBLive.GetSchema("DataTypes");

			var res= new List<DataTypeInfo>();
			foreach(DataRow t in DataTypesSchema.Rows)
			{
				var r = new DataTypeInfo
				{
					TypeName = t.getString("TypeName",""),
					DataType = t.getString("DataType", ""),
					CreateFormat = t.getString("CreateFormat", ""),
					CreateParameters = t.getString("CreateParameters", ""),
					ProviderDbType = t.getInt("ProviderDbType",0),
				};
				res.Add(r);
            }
			return res;
