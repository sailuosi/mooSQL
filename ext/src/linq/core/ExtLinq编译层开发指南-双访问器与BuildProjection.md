# Ext LINQ 编译层开发指南：双访问器与 BuildProjection
...
| `IClauseContext` | `clauseContext/*` | 维护上述 AST 与当前序列的投影、别名、父子关系 |
| `StatementExpression` | 编译链上的扩展表达式节点 | 把成功的 `IClauseContext` 折叠回表达式树 |
...
编译成功后，算子不会直接返回 `IClauseContext`，而是包装为表达式树上的节点，便于 Buddy 递归：

```
ClauseMethodVisitor.VisitWhere(...)
  → BuildWhere(...) 得到 IClauseContext
...
| 类型 | 作用 |
|------|------|
| `StatementExpression` | 携带 `IClauseContext`；`ToStatement()` → `BaseSentence` |
...
protected IClauseContext? ResolveSourceContext(MethodCallExpression methodCall, BuildInfo buildInfo, ...)
{
...
`TableContext` 即第一个 `IClauseContext`，其 `SelectQuery` 指针指向上述 AST。
...
## 5. IClauseContext 与 SelectQueryClause（SQL 模型）

### 5.1 IClauseContext 是什么

`IClauseContext` 是 **「当前 LINQ 序列段」的编译环境**：

```csharp
// ext/src/linq/src/linq/builder/interfaces/IClauseContext.cs
...
```csharp
// ext/src/linq/src/linq/builder/clauseContext/BuildInfo.cs
...
## 6. BuildProjection：Lambda 路径解析引擎

### 6.1 含义

`BuildProjection` 负责回答：

> **在当前 IClauseContext 下，Lambda 里的某个表达式片段（path）对应 SQL 模型里的什么？**
...
```csharp
// ext/src/linq/src/linq/builder/clauseSqlTranslator/ClauseSqlTranslator.SqlBuilder.BuildProjection.cs
...
```csharp
// ext/src/linq/src/linq/builder/clauseContext/TableContext.cs
...
| 方法 | 层级 | 作用 |
|------|------|------|
| **`BuildProjection`** | 表达式树 | 路径 → 上下文绑定的中间节点 |
       ...
       → ClauseSqlTranslator.BuildSearchCondition
            → ConvertPredicate / ConvertCompareExpression
                 → ConvertToSql(sequence, u.Age)
                      → BuildProjection(context, u.Age, ProjectFlags.SQL)   // ★ 先解析路径
                      → 列占位符 → IExpWord
                 → ConvertToSql(..., 20)
                 → 合成 IAffirmWord(>)
...
`SelectContext` 在 `CompleteColumns` / `BuildProjection` 中把投影 Lambda 的每个成员解析为 `Select.Columns` 条目。
...
VisitXxx(MethodCall method)
  ...
  5. （Context 内或 Translator 内）BuildProjection / ConvertToSql 解析 Lambda
...
3. 如需新状态，在 `clauseContext/` 添加 `XxxContext`
4. 复杂 Lambda 解析复用 `ClauseSqlTranslator.BuildProjection` / `ConvertToSql`
...
flowchart TB
    ...
    subgraph me [BuildProjection 层]
        ME[BuildProjection path, flags]
...
- 谓词路径 `BuildProjection` 失败（成员未映射、不支持的 MethodCall）
...
| 改什么 | 去哪 |
|--------|------|
| 新增 LINQ 算子 | `ClauseMethodVisitor.Xxx.cs` + 可选 `clauseContext/XxxContext.cs` |
| Lambda 成员 → 列 | `TableContext.BuildProjection` / `ClauseSqlTranslator.BuildProjection` |
| 谓词 / Like / InList | `ClausePredicateVisitor` + `ClauseSqlTranslator.SqlBuilder.Predicate.cs` |
| 关联导航 | `BuildProjection` 中 `TryCreateAssociation` |
...
| BuildProjection | `.../ClauseSqlTranslator.SqlBuilder.BuildProjection.cs` |
| ConvertToSql | `.../ClauseSqlTranslator.SqlBuilder.BuildExpression.cs` |
| IClauseContext | `ext/src/linq/src/linq/builder/clauseContext/` |
