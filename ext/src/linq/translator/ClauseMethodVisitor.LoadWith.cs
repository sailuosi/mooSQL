...
internal partial class ClauseMethodVisitor
{
    ...
    MethodCall VisitLoadWithCore(MethodCall method)
    {
        ...
        if (!CanBuildLoadWith(methodCall))
            return method;

        return ToStatementCallOr(method, BuildLoadWith(Context.Builder, buildInfo, methodCall));
    }

    static bool CanBuildLoadWith(MethodCallExpression call)
        => call.IsQueryable();

    static IBuildContext? BuildLoadWith(ClauseSqlTranslator builder, BuildInfo buildInfo, MethodCallExpression methodCall)
    {
