...
| **Ext LINQ** | **对标 EF Core、Linq2DB、SqlSugar Queryable** 等通用能力 | `useQueryable` / `AsQueryable` → `ITable<T>` | `EntityLinqFactory` / `EntityVisitFactory` → `EntityQueryCompiler` / `EntityVisitCompiler` | 能力对齐、Statement 级测试、Queryable 生态 |
...
| **Ext 入口是什么？** | **`useQueryable` / `AsQueryable`**（`GetTable` 为 Linq2DB 兼容），对标 EF 与其他 ORM |
