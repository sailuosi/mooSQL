	...
	partial class Sql
	{
		...
		public class ExpressionAttribute : MappingAttribute
		{
			...
			public ExpressionAttribute(string? expression)
			{
				...
				Precedence = PrecedenceLv.Primary;
				IsPure     = true;
			}
			...
			/// Refer to <see cref="PrecedenceLv"/>.
			/// </summary>
			...
			public static readonly ExpressionWord UnknownExpression = new ("!!!");
			...
			public static void PrepareParameterValues<TContext>(
				...
				out List<DataTypeWord>?                                                genericTypes,
				ConvertFunc<TContext>                                                 converter)
			{
				...
				if (expression.NodeType == ExpressionType.Call)
				{
					...
					if (!ignoreGenericParameters)
					{
						...
						{
							genericTypes = new List<DataTypeWord>();
							foreach (var t in mc.Method.DeclaringType.GetGenericArguments())
							{
								//var type = mappingSchema.dialect.mapping.GetDataType(t);
			...
			public delegate Expression ConvertFunc<TContext>(TContext context, Expression expression, EntityColumn? columnDescriptor, bool? inlineParameters);

			public static IExpWord?[] PrepareArguments<TContext>(TContext   context,
				string                                                            expressionStr,
				int[]?                                                            argIndices,
				bool                                                              addDefault,
				List<(Expression? expression, ExprParameterAttribute? parameter)> knownExpressions,
				List<DataTypeWord>?                                                genericTypes,
				ConvertFunc<TContext>                                             converter,
				out Expression?                                                   error)
			{
				var parms = new List<IExpWord?>();
				...
				ConvertFunc<TContext> converter)
			{
				...
				var sqlExpression = new ExpressionWord(expression.Type, expressionStr!, Precedence,
