	...
	public static partial class Sql
	{
		...
		sealed class StringAggSql2017Builder : IExtensionCallBuilder
		{
			public void Build(ISqExtensionBuilder builder)
			{
				...
				if (descriptor != null)
				{
					var dbDataType = descriptor.DbType;
					if (dbDataType.DataType != DataFam.Undefined)
					{
						var separator = builder.GetExpression("separator");

						if (separator is ValueWord value && value.ValueType.DataType == DataFam.Undefined)
							value.ValueType = value.ValueType.WithDataType(dbDataType.DataType);
						else if (separator is ParameterWord parameter && parameter.Type.DataType == DataFam.Undefined)
		...
		sealed class StringAggSapHanaBuilder : IExtensionCallBuilder
		{
			public void Build(ISqExtensionBuilder builder)
			{
				...
				if (separator is ParameterWord parameter)
		...
		sealed class CommonConcatWsArgumentsBuilder : IExtensionCallBuilder
		{
			...
			public void Build(ISqExtensionBuilder builder)
			{
				...
				{
					builder.ResultExpression = new ExpressionWord(typeof(string), "''");
				}
		...
		{
			protected override ExpressionWord IsNullExpression(IExpWord value)
			{
				return new ExpressionWord(typeof(string), "ISNULL({0}, '')", PrecedenceLv.Primary, value);
			}

			protected override ExpressionWord StringConcatExpression(IExpWord value1, IExpWord value2)
			{
				return new ExpressionWord(typeof(string), "{0} + {1}", value1, value2);
			}

			protected override ExpressionWord TruncateExpression(IExpWord value, IExpWord separator)
			{
				// you can read more about this gore code here:
				// https://stackoverflow.com/questions/2025585
				return new ExpressionWord(typeof(string), "SUBSTRING({0}, LEN(CONVERT(NVARCHAR(MAX), {1}) + N'!'), 8000)",
					PrecedenceLv.Primary, value, separator);
			}
		}
		...
		sealed class SqliteConcatWsBuilder : BaseEmulationConcatWsBuilder
		{
			protected override ExpressionWord IsNullExpression(IExpWord value)
			{
				return new ExpressionWord(typeof(string), "IFNULL({0}, '')", PrecedenceLv.Primary, value);
			}
			...
			protected override ExpressionWord TruncateExpression(IExpWord value, IExpWord separator)
			{
				return new ExpressionWord(typeof(string), "SUBSTR({0}, LENGTH({1}) + 1)",
					PrecedenceLv.Primary, value, separator);
			}
		}
