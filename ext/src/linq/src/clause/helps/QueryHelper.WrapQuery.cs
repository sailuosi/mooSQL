...
namespace mooSQL.linq.SqlQuery
{
	...
	public partial class QueryHelper
	{
		class WrapQueryVisitor<TContext> : SqlQueryVisitor
		{
			...
			public Func<TContext, SelectQueryClause, ISQLNode?, int> WrapTest { get; }
			public Action<TContext, IReadOnlyList<SelectQueryClause>>     OnWrap   { get; }

			public WrapQueryVisitor(
				VisitMode                                        visitMode,
				TContext                                         context,
				Func<TContext, SelectQueryClause, ISQLNode?, int> wrapTest,
				Action<TContext, IReadOnlyList<SelectQueryClause>>     onWrap
				) : base(visitMode, null)
			{
				Context  = context;
				WrapTest = wrapTest;
			...
			public override Clause ProcessElement(Clause element)
			{
				_root = element;
			...
			{
				selectQuery = (SelectQueryClause)base.VisitSelectQuery(selectQuery);

				var ec = WrapTest(Context, selectQuery, null);
				if (ec <= 0)
					return selectQuery;

				var queries = new List<SelectQueryClause>(ec);
				for (int i = 0; i < ec; i++)
				{
				...
				{
					queries[i].From.FindTableSrc(queries[i + 1]);
				}
		...
		public static TStatement WrapQuery<TStatement, TContext>(
			TContext                                          context,
			TStatement                                        statement,
			Func<TContext, SelectQueryClause, ISQLNode?, bool> wrapTest,
			Action<TContext, SelectQueryClause, SelectQueryClause>?       onWrap,
			bool                                              allowMutation,
			bool                                              withStack)
			where TStatement : BaseSentence
		{
