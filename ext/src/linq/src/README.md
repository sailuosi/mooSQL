...
- Queryable 扩展（Includes、Merge、SetOp、InsertOrUpdate 等）
...
- 实体映射、导航属性（`Includes`）、`DbFunc` 方言函数等衍生能力
...
│    → IClauseContext + ClauseSqlTranslator（工具，非编排器）      │
...
MethodCallExpression
    ...
    → IClauseContext → StatementExpression
```
...
| `ClauseSqlTranslator` | SQL 语义引擎（BuildProjection / ConvertToSql / BuildWhere 等） |
...
新增 LINQ 方法时：在 `ClauseMethodVisitor.*.cs` 添加 `VisitXxx` + `BuildXxxCore`；Context 类放 `clauseContext/`。

### IClauseContext 与编译后端（保留）

- `IClauseContext` + `clauseContext/*`：维护 `SelectQueryClause`、投影、`BuildProjection`
- `ClauseSqlTranslator`：SQL 工具（BuildProjection / ConvertToSql / BuildWhere 等）
...
├── NavColumns: Dict<Type, List<EntityColumn>>  // Includes 导航列
├── buildContext: IClauseContext       // 编译上下文（调试 / 二次投影）
...
DefaultGetResultEnumerable  → StreamingResultEnumerable（无 Includes）/ MaterializedResultEnumerable
```
...
`Includes` 在编译阶段向 `SentenceBag.NavColumns` 注册导航列；主查询执行后由 `NavColumnLoader.LoadNavChilds` 发起二次 IN 查询，支持多级 Includes 与 `HashSet<Type>` 循环引用检测。
...
│   NavColumnLoader.cs       # Includes 二次加载
...
└── src/
    ...
    ├── linq/                # 面向用户的 LINQ 核心
    │   ├── builder/         # ClauseSqlTranslator + clauseContext/ + IClauseContext
    ...
    │   └── metadata/
    └── api/             # 历史 API、QueryRunner 缓存、Sql 扩展方法
```
...
| `IClauseContext` | 编译 + `SetRunQuery` 绑定执行 | **仅编译**（`BuildProjection` / `GetResultStatement`） |
...
| Includes* | ✅ | `ClauseMethodVisitor.Includes.cs` |
...
新增算子时在 `ClauseMethodVisitor.*.cs` 添加 `VisitXxx` + `BuildXxxCore`；Context 类放 `clauseContext/`。
...
**保留：** `api/` 下 `LinqExtensions`、`Sql` 等仍为公共 API，后续可 rename 为 `publicapi/`。

---

## 实现进度总览（2026-06 比对）

Phase 2 三层架构（Compile → SentenceBag → Execute）**主骨架已落地**。`BuildProjection` 已从 Clause 编译；SQLite 端到端 Where 查询已验证。

| 层级 | 状态 | 说明 |
|------|------|------|
| Layer 1 Compile | ✅ | 双访问器 + `ClauseCompiler`；`BuildProjection` 已移植 |
...
## Phase D / E 路线图（DbFunc → Pure）

R0–**R6 已完成**（注册表翻译、Union、Select 投影、`api/dbfunc/` 目录收缩、**68/68** 测试）。  
下一批 **R7** 重点：注册表扩展、RowNumber 端到端、SQLClip 互操作测。

详见 **[`core/Phase-D-E-Roadmap.md`](../core/Phase-D-E-Roadmap.md)**。
...
- [x] **`ExpressionBuilder.BuildProjection`** — Clause 编译层实现至 `ExpressionBuilder.SqlBuilder.BuildProjection.cs`
...
- [ ] **`ExpressionBuilder.GetContext`** — 低优先级 stub
...
- [x] `NavColumnLoader`：多级 Includes、循环引用检测
...
- [ ] 与 **SQLClip / SQLBuilder** 链式 API 互操作（Expression ↔ SQLBuilder 双向）— 正向/逆向桥接已有，缺 SQLClip 往返测（R7）
...
```
BuildProjection + GetSubQuery（已完成）
...
- `ext/src/linq/core/ExtLinq编译层开发指南-双访问器与BuildProjection.md` — **开发人员**：Buddy 双访问器、`BuildProjection`、Expression → `SelectQueryClause` 走读
