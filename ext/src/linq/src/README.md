...
**集成测试（SQLite）：** `LinqSqliteTestFixture` + `LINQTest.useBus1` + `LinqCompileTests`（Where/OrderBy/Take/Count 编译与执行已启用；关联执行仍 Skip）。
...
MakeExpression + GetSubQuery（已完成）
  ...
  → OrderBy / Take / Count 编译与执行链 ✅（`ParametersContext`：`UnderType` 空时回退 `PropertyInfo.PropertyType`）
