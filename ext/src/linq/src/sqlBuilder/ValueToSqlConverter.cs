...
namespace mooSQL.linq.SqlProvider
{
    ...
    using mooSQL.utils;
    using ConverterType = Action<StringBuilder, data.model.DbDataType, mooSQL.data.DBInstance,object>;

	public class ValueToSqlConverter
	{
		public ValueToSqlConverter()
		{
			BaseConverters = new ValueToSqlConverter[] { };
		}

		public ValueToSqlConverter(params ValueToSqlConverter[]? converters)
		{
			BaseConverters = converters ?? new ValueToSqlConverter[] { };
		}
		...
		public bool TryConvert(StringBuilder stringBuilder, DBInstance mappingSchema,  object? value)
		{
			return TryConvert(stringBuilder, mappingSchema, null,  value);
		}

		public bool TryConvert(StringBuilder stringBuilder, DBInstance db, DbDataType? dataType, object? value)
		{
			...
			return TryConvertImpl(stringBuilder, dataType ?? db.dialect.mapping.GetDbDataType(value.GetType()), db, value, true);
		}

		public bool CanConvert(DbDataType dataType, DBInstance db, object? value)
		{
			return TryConvertImpl(null, dataType, db, value, true);
		}

		bool TryConvertImpl(StringBuilder? stringBuilder, DbDataType dataType, DBInstance options, object? value, bool tryBase)
		{
			if (value == null || value is INullable nullable && nullable.IsNull)
			{
				stringBuilder?.Append("NULL");
		...
		public StringBuilder Convert(StringBuilder stringBuilder, DBInstance mappingSchema,  object? value)
		{
