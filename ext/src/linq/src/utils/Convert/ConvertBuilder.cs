...
namespace mooSQL.linq.Common
{
	...
	public static class ConvertBuilder
	{
		...
		static object ConvertDefault(object value, Type conversionType)
		{
			...
			{
				throw new Exception($"Cannot convert value '{value}: {value.GetType().FullName}' to type '{conversionType.FullName}'", ex);
			}
		}
		...
		static Expression? GetValueOrDefault(Type from, Type to, Expression p)
		{
			...
			var mi = from.GetMethod("GetValueOrDefault", BindingFlags.Instance | BindingFlags.Public, null,new Type [] { }, null);
		...
		static Expression? GetToString(Type from, Type to, Expression p)
		{
			if (to == typeof(string) && !from.IsNullable())
			{
				var mi = from.GetMethodEx("ToString",new Type[] { });
		...
		static Tuple<Expression,bool>? GetConverter(DBInstance mappingSchema, Expression expr, Type from, Type to)
		{
			if (from == to)
			...
			var lex =TypeConverterUtil.GetConvertType( mappingSchema,from, to);

			if (lex != null)
				return Tuple.Create(lex.GetBody(expr), true);

			var cex = TypeConverterUtil.GetConvertType(mappingSchema, from, to);
		...
		public static Tuple<LambdaExpression,LambdaExpression?,bool> GetConverter(DBInstance? mappingSchema, Type from, Type to)
		{
			...
			var ex =
                ...
                ConvertUnderlying(mappingSchema, p, from, from.UnwrapNullable(), to, to.UnwrapNullable()) ??
                ConvertUnderlying(mappingSchema, p, from, NullTypeExtensions.UnwrapNullable(from),         to, NullTypeExtensions.UnwrapNullable(to));
		...
		public static Type? GetDefaultMappingFromEnumType(DBInstance DB, Type enumType)
		{
			var type = enumType.UnwrapNullable();
			...
			if ((enumType.IsNullable() || hasNullValue) && !defaultType.IsReferType())
				defaultType = ReflectionExtensions.WrapNullable(defaultType);
