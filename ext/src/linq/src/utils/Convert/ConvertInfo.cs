...
namespace mooSQL.linq.Common
{
	...
	sealed class ConvertInfo
	{
		...
		public LambdaInfo Create(DBInstance? mappingSchema, Type from, Type to, ConversionType conversionType)
		{
			return Create(mappingSchema, new DbDataType(from), new DbDataType(to), conversionType);
		}

		public LambdaInfo Create(DBInstance? mappingSchema, DbDataType from, DbDataType to, ConversionType conversionType)
		{
