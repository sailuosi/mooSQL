...
namespace mooSQL.linq.Reflection
{
    ...
    public class MemberAccessor
	{
		...
		internal MemberAccessor(TypeAccessor typeAccessor, string memberName, EntityInfo? ed)
		{
			...
			else
			{
				...
				var checkNull = infos.Take(infos.Length - 1).Any(info => info.type.IsReferType());
				...
				if (checkNull)
				{
					...
					expr = Expression.Block(
						new[] { ret },
						Expression.Assign(ret, new DefaultValueExpression( Type)),
		...
		public MemberAccessor(TypeAccessor typeAccessor, MemberInfo memberInfo, EntityInfo? ed)
		{
			TypeAccessor = typeAccessor;
...
#pragma warning restore CS3016 // Arrays as attribute arguments is not CLS-compliant
		void SetSimple(MemberInfo memberInfo, EntityInfo? ed)
		{
			...
			//else
				//_getterExpression = new DefaultValueExpression(Type);
