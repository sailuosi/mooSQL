    ...
    static class InternalExtensions
	{
		...
		public static bool IsNullValue(this Expression expr)
		{
			return (expr is ConstantExpression { Value: null })
				|| (expr is DefaultExpression or DefaultValueExpression && expr.Type.IsReferType());
		}
		...
		public static bool IsEvaluable(this Expression? expression, DBInstance mappingSchema)
		{
			return expression?.NodeType switch
			{
				...
				ExpressionType.MemberAccess => IsEvaluable(((MemberExpression)expression).Expression, mappingSchema),
				_                           => false,
			};
		}
		...
		public static Expression? OptimizeExpression(this Expression? expression, DBInstance DB)
		{
			return TransformInfoVisitor<DBInstance>.Create(DB, OptimizeExpressionTransformer).Transform(expression);
		}

		private static TransformInfo OptimizeExpressionTransformer(DBInstance DB, Expression e)
		{
			var newExpr = e;
			if (e is BinaryExpression binary)
			{
				var left  = OptimizeExpression(binary.Left, DB)!;
				var right = OptimizeExpression(binary.Right, DB)!;
				...
				newExpr = binary.Update(left, OptimizeExpression(binary.Conversion, DB) as LambdaExpression, right);
			}
			else if (e is UnaryExpression unaryExpression)
			{
				newExpr = unaryExpression.Update(OptimizeExpression(unaryExpression.Operand, DB));
			...
			else
			{
				switch (newExpr)
				{
					...
					case UnaryExpression unary when IsEvaluable(unary.Operand, DB):
					{
						newExpr = Expression.Constant(unary.EvaluateExpression());
						break;
					}
					case MemberExpression { Expression.NodeType: ExpressionType.Constant } me when IsEvaluable(me.Expression, DB):
					{
						newExpr = Expression.Constant(me.EvaluateExpression());
						break;
					}
					case BinaryExpression be when IsEvaluable(be.Left, DB) && IsEvaluable(be.Right, DB):
					{
					...
					case BinaryExpression { NodeType: ExpressionType.AndAlso } be:
					{
						...
						else if (IsEvaluable(be.Right, DB))
						{
