...
namespace mooSQL.linq.Linq.Builder.Visitors
{
    ...
    class ExposeExpressionVisitor : ExpressionVisitorBase, IExpressionEvaluator
	{
		...
		public Expression ExposeExpression(DBInstance dataContext,
		...
		protected override Expression VisitMethodCall(MethodCallExpression node)
		{
			...
			if (node.Method.IsSqlPropertyMethodEx())
			{
				...
				var en = this.DBLive.client.EntityCash.getEntityInfo(entity.Type);

				var col = en.GetColumn(memberName);
				MemberInfo memberInfo=null;
				if (col == null)
				{
					if (col.PropertyInfo is MemberInfo mem) {
                        memberInfo = mem;
                    }
				}
		...
		Expression? ConvertMethod(MethodCallExpression pi)
		{
			...
			if (!pi.Method.IsStatic && pi.Object != null && pi.Object.Type != pi.Method.DeclaringType)
			{
				...
				if (concreteTypeMemberInfo != null)
					lambda = Expressions.ConvertMember(DBLive, pi.Object.Type, concreteTypeMemberInfo);
			}

			lambda ??= Expressions.ConvertMember(DBLive, pi.Object?.Type, pi.Method);
		...
		{
			var l = Expressions.ConvertBinary(DBLive, node);
			if (l != null)
			{
						...
						if (mc.Arguments[0] is MemberExpression or ConstantExpression)
						{
							if (IsCompilable(mc))
							{
								...
								if (evaluated != null)
								{
									...
									{
										if (evaluated is DBInstance dc)
										{
											var args = mc.Arguments.ToArray();
											args[0]   = SqlQueryRootExpression.Create(DBLive, evaluatedType);
		...
		protected override Expression VisitMember(MemberExpression node)
		{
			if (!IsCompilable(node))
			{
				...
				if (l != null)
				{
					var converted = ConvertMemberExpression(node, DBLive, node.Expression!, l);
					converted = Visit(converted);
					return AliasCall(converted, alias);
				}
			}
			...
			{
				if (typeof(IQueryable).IsSameOrParentOf(node.Type) && typeof(DBInstance).IsSameOrParentOf(node.Expression.Type))
				{
		...
		{
			var l = Expressions.ConvertMember(DBLive, node.Expression?.Type, node.Member);
		...
		protected override Expression VisitUnary(UnaryExpression node)
		{
			switch (node.NodeType)
			{
				...
				{
					var ll = Expressions.ConvertMember(DBLive, node.Operand?.Type, node.Operand!.Type.GetProperty(nameof(Array.Length))!);
					if (ll != null)
					{
						var exposed = ConvertMemberExpression(node, DBLive, node.Operand!, ll);
		...
		Expression? ConvertNew(NewExpression pi)
		{
			...
			var lambda = Expressions.ConvertMember(DBLive, pi.Type, pi.Constructor);
		...
		public Expression ConvertMethod(MethodCallExpression node, LambdaExpression replacementLambda)
		{
			...
			var newNode = replacementBody.Transform((node, parms, DBLive), static (context, wpi) =>
			{
				if (wpi.NodeType == ExpressionType.Parameter)
				{
					if (context.parms.TryGetValue((ParameterExpression)wpi, out var n))
					{
						...
						{
							if (typeof(DBInstance).IsSameOrParentOf(wpi.Type))
							{
		...
		static Expression ConvertMemberExpression(Expression expr, DBInstance DB, Expression root, LambdaExpression l)
		{
			var body  = l.Body.Unwrap();
