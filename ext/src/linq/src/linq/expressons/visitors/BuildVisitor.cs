...
namespace mooSQL.linq.Linq.Builder
{
    ...
    sealed class BuildVisitor : ExpressionVisitorBase
	{
		...
		EntityColumn? _entityColumn { get; set; }
		...
		Expression TranslateExpression(Expression expression, string? alias = null, bool useSql = false)
		{
			...
			{
				if (!Builder.DBLive.dialect.mapping.IsScalarType(expression.Type))
				{
		...
		protected override MemberAssignment VisitMemberAssignment(MemberAssignment node)
		{
			...
			{
				_entityColumn = DBLive.client.EntityCash.getEntityInfo(node.Member.DeclaringType).GetColumn(node.Member);
			}
		...
		internal override SqlGenericConstructorExpression.Assignment VisitSqlGenericAssignment(SqlGenericConstructorExpression.Assignment assignment)
		{
			...
			{
                _entityColumn = DBLive.client.EntityCash.getEntityInfo(assignment.MemberInfo.DeclaringType).GetColumn(assignment.MemberInfo);
            }
		...
		internal override SqlGenericConstructorExpression.Parameter VisitSqlGenericParameter(SqlGenericConstructorExpression.Parameter parameter)
		{
			...
			{
                _entityColumn = DBLive.client.EntityCash.getEntityInfo(parameter.MemberInfo.DeclaringType).GetColumn(parameter.MemberInfo);
            }
		...
		{
			var translatedMemberExpression = Builder.TranslateMember(_context, _flags, _entityColumn, _alias, node);
		...
		{
			return Builder.ParseGenericConstructor(expression, _flags, _entityColumn);
		}
		...
		protected override Expression VisitMethodCall(MethodCallExpression node)
		{
			...
			var translatedMethodCall = Builder.TranslateMember(_context, localFlags, _entityColumn, _alias, node);
		...
		bool HandleParameterized(Expression expr, [NotNullWhen(true)] out Expression? transformed)
		{
			...
			//
			if (canBeCompiled && Builder.IsForceParameter(expr, _entityColumn))
			{
		...
		public override Expression VisitSqlDefaultIfEmptyExpression(SqlDefaultIfEmptyExpression node)
		{
			...
			if (_flags.IsExpression() && _buildFlags.HasFlag(BuildFlags.ForceDefaultIfEmpty))
			{
				...
				var defaultValue = new DefaultValueExpression(DBLive, innerExpression.Type);
