...
namespace mooSQL.linq.Linq.Builder
{
	...
	sealed class FirstSingleBuilder : MethodCallBuilder
	{
		...
		public sealed class FirstSingleContext : SequenceContextBase
		{
			...
			static void GetFirstElement<T>(SentenceBag<T> query)
			{
				query.Runner.whenGetElement( (cont) =>
					query.Runner.loadResultList(cont).First());

				query.Runner.whenGetElementAsync(  async (cont) =>

#if NET6_0_OR_GREATER
					await query.Runner.loadResultList(cont)
                        .FirstAsync(cont.cancellationToken).ConfigureAwait(Configuration.ContinueOnCapturedContext));
#else
                    query.Runner.loadResultList(cont).First());
#endif
			...
			static void GetFirstOrDefaultElement<T>(SentenceBag<T> query)
			{
				query.Runner.whenGetElement( (cont) =>
					query.Runner.loadResultList(cont).FirstOrDefault());

				query.Runner.whenGetElementAsync(async (cont) =>
#if NET6_0_OR_GREATER
                    await query.Runner.loadResultList(cont)
						.FirstOrDefaultAsync(cont.cancellationToken).ConfigureAwait(Configuration.ContinueOnCapturedContext));
#else
				query.Runner.loadResultList(cont).FirstOrDefault());
#endif
			...
			static void GetSingleElement<T>(SentenceBag<T> query)
			{
				query.Runner.whenGetElement( (cont) =>
					query.Runner.loadResultList(cont).Single());

				query.Runner.whenGetElementAsync(async (cont) =>
#if NET6_0_OR_GREATER
                    await query.Runner.loadResultList(cont)
						.SingleAsync(cont.cancellationToken).ConfigureAwait(Configuration.ContinueOnCapturedContext));
#else
				query.Runner.loadResultList(cont).Single());
#endif
			...
			static void GetSingleOrDefaultElement<T>(SentenceBag<T> query)
			{
				query.Runner.whenGetElement( (cont) =>
					query.Runner.loadResultList(cont).SingleOrDefault());

				query.Runner.whenGetElementAsync(async (cont) =>
#if NET6_0_OR_GREATER
                    await query.Runner.loadResultList(cont)
						.SingleOrDefaultAsync(cont.cancellationToken).ConfigureAwait(Configuration.ContinueOnCapturedContext));
#else
				query.Runner.loadResultList(cont).SingleOrDefault());
#endif
