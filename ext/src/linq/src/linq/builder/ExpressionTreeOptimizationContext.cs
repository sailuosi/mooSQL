...
namespace mooSQL.linq.Linq.Builder
{
    ...
    public class ExpressionTreeOptimizationContext
	{
		...
		public ExpressionTreeOptimizationContext(DBInstance dataContext)
		{
			DBLive = dataContext;
		...
		internal EqualsToVisitor.EqualsToInfo GetSimpleEqualsToContext(bool compareConstantValues)
		{
			if (compareConstantValues)
			{
				if (_equalsToContextTrue == null)
					_equalsToContextTrue = EqualsToVisitor.PrepareEqualsInfo(DBLive, compareConstantValues : compareConstantValues);
				else
			...
			else
			{
				if (_equalsToContextFalse == null)
					_equalsToContextFalse = EqualsToVisitor.PrepareEqualsInfo(DBLive, compareConstantValues : compareConstantValues);
				else
		...
		public TransformInfo OptimizeExpressionTreeTransformer(Expression expr, bool inProjection)
		{
			bool IsEqualConstants(Expression left, Expression right)
			{
				var valueLeft = left switch
				{
					...
					SqlPlaceholderExpression { Sql: ValueWord leftConst } => leftConst.Value,
					_ => DBLive.dialect.mapping.GetDefaultValue(left.Type),
				};

				var valueRight = right switch
				{
					ConstantExpression rightConst                         => rightConst.Value,
					SqlPlaceholderExpression { Sql: ValueWord rightConst } => rightConst.Value,
					_ => DBLive.dialect.mapping.GetDefaultValue(right.Type),
				};
		...
		class IsServerSideOnlyCheckVisitor : ExpressionVisitorBase
		{
			...
			DBInstance                                        _mappingSchema  = default!;
			...
			public bool IsServerSideOnly(Expression expression, DBInstance mappingSchema)
			{
				Cleanup();
		...
		public bool IsServerSideOnly(Expression expr, bool inProjection)
		{
			...
			else
			{
				...
				result = visitor.Value.IsServerSideOnly(expr, DBLive);
			}
		...
		class CanBeCompiledCheckVisitor : ExpressionVisitorBase
		{
			...
			public bool CanBeCompiled(Expression expression, DBInstance mappingSchema, ExpressionTreeOptimizationContext optimizationContext, bool inProjection)
			{
				Cleanup();
			...
			protected override Expression VisitMethodCall(MethodCallExpression node)
			{
				...
				{
					if (node.Arguments.Any(a => typeof(DBInstance).IsSameOrParentOf(a.Type)) ||
					    node.Object != null && typeof(DBInstance).IsSameOrParentOf(node.Object.Type))
					{
						CanBeCompiledFlag = false;
						return node;
					}
		...
		public bool CanBeCompiled(Expression expr, bool inProjection)
		{
			...
			var result = visitor.Value.CanBeCompiled(expr, DBLive, this, inProjection);
		...
		private bool CanBeConstantFind(Expression ex)
		{
			...
			switch (ex.NodeType)
			{
				...
				case ExpressionType.Convert:
				{
					...
					if (unary.Operand.Type.IsReferType() && !unary.Type.IsReferType())
						return true;
				...
				case ExpressionType.MemberAccess:
				{
					...
					var l = Expressions.ConvertMember(DBLive, ma.Expression?.Type, ma.Member);
