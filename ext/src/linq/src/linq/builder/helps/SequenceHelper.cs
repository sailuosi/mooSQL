...
namespace mooSQL.linq.Linq.Builder
{
	...
	internal static class SequenceHelper
	{
			...
			if (!ReferenceEquals(body, lambda.Body))
			{
				body = body.Transform(e =>
				{
					if (e.NodeType is ExpressionType.Convert or ExpressionType.ConvertChecked
						...
						&& !e.Type.UnwrapNullable().IsValueType)
					{
		...
		public static Expression CorrectSelectQuery(Expression expression, SelectQueryClause selectQuery)
		{
		...
		public static Expression StampNullability(Expression expression, SelectQueryClause query)
		{
			...
			var translated = expression.Transform(e =>
			{
				...
				{
					placeholder = placeholder.WithSql(NullabilityWord.ApplyNullability(placeholder.Sql, nullability));
					return placeholder;
				}
		...
		public static IExpWord UnwrapNullability(IExpWord expression)
		{
			while (expression is NullabilityWord nullability)
			{
		...
		public static ITableNode? GetExpressionSource(IExpWord expression)
		{
			return expression switch
			{
                ColumnWord column => column.Parent,
                FieldWord field   => field.Table,
				_                => null,
			};
		}
		...
		public static bool HasDependencyWithOuter(SelectQueryClause selectQuery)
		{
		...
		public static Expression MakeNotNullCondition(Expression expr)
		{
			if (expr.Type.IsValueType && !expr.Type.IsNullable())
			{
				if (expr is SqlPlaceholderExpression placeholder)
					expr = placeholder.WithSql(NullabilityWord.ApplyNullability(placeholder.Sql, true)).MakeNullable();
				else
					expr = Expression.Convert(expr, expr.Type.WrapNullable());
			}
