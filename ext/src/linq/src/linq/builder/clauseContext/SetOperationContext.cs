	...
	internal sealed class SetOperationContext : SubQueryContext
	{
		...
		Expression? GetDifferencePredicateConstants(bool isLeft)
		{
			foreach (var map in _pathMapping)
			{
				...
				if (placeholder1.Sql is ColumnWord column1 && placeholder2.Sql is ColumnWord column2)
				{
					if (column1.Expression is ColumnWord { Expression: ValueWord sqlValue1 } && column2.Expression is ColumnWord { Expression: ValueWord sqlValue2 })
					{
		...
		static Expression MakeNullCondition(SqlPathExpression path, bool isNotNull)
		{
			...
			{
				path = path.WithType(path.Type.WrapNullable());
			}
		...
		bool TryMergeProjections(Expression projection1, Expression projection2, ProjectFlags flags, [NotNullWhen(true)] out Expression? merged)
		{
			    ...
			    SequenceHelper.UnwrapDefaultIfEmpty(projection2) is SqlGenericConstructorExpression generic2)
			{
				if (generic1.ConstructType == SqlGenericConstructorExpression.CreateType.Full)
				{
					...
					{
						var constructed = Builder.TryConstruct(DB, generic1, flags);
						if (constructed == null)
							return false;
				...
				if (generic2.ConstructType == SqlGenericConstructorExpression.CreateType.Full)
				{
					if (generic1.ConstructType != SqlGenericConstructorExpression.CreateType.Full)
					{
						var constructed = Builder.TryConstruct(DB, generic2, flags);
						if (constructed == null)
							return false;
				...
				if (Builder.TryConstruct(DB, resultGeneric, flags) == null)
					return false;
		...
		static bool ExtractValue(SqlPlaceholderExpression placeholder, [NotNullWhen(true)] out ValueWord? sqlValue)
		{
			if (placeholder.Sql is ColumnWord column && QueryHelper.UnwrapNullablity(column.Expression) is ValueWord sqlValue1)
			{
				sqlValue = sqlValue1;
				return true;
			}
			...
			foreach (var (placeholder, path) in placeholders1)
			{
				...
				else
				{
					if (ExtractValue(placeholder1, out var sqlValue1) && !ExtractValue(placeholder2, out _))
						sqlValue1.ValueType = QueryHelper.GetDbDataType(placeholder2.Sql, DB);
					else
					{
						if (ExtractValue(placeholder2, out var sqlValue2) && !ExtractValue(placeholder1, out _))
							sqlValue2.ValueType = QueryHelper.GetDbDataType(placeholder1.Sql, DB);
					}
		...
		void EnsureSetIdFieldCreated()
		{
			...
			var sqlValueLeft  = new ValueWord(_leftSetId!);
			var sqlValueRight = new ValueWord(_rightSetId!);
		...
		Expression BuildProjectionExpression(Expression path, IClauseContext context,
			...
			out List<SqlEagerLoadExpression> foundEager)
		{
			...
			do
			{
				var projected = Builder.BuildSqlExpression(context, current, ProjectFlags.Expression,
					buildFlags : BuildFlags.ForceAssignments | BuildFlags.ForceDefaultIfEmpty);
		...
		public IClauseContext Emulate()
		{
			...
			for (int i = 0; i < _sequence1.SelectQuery.Select.Columns.Count; i++)
			{
				...
				sc.AddEqual(column1.Expression, column2.Expression,DB.dialect.Option.CompareNullsAsValues);
			}
