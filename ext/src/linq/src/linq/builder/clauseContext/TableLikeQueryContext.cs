	...
	sealed class TableLikeQueryContext : ClauseContextBase
	{
			...
			: base(sourceContextRef.BuildContext.Builder, targetContextRef.ElementType, sourceContextRef.BuildContext.SelectQuery)
		{
			...
			Source = sourceContextRef.BuildContext is EnumerableContext enumerableSource
				? new TableLikeSourceWord { SourceEnumerable = enumerableSource.Table }
				: new TableLikeSourceWord { SourceQuery = sourceContextRef.BuildContext.SelectQuery };
		}
			...
			if (!flags.IsTest())
			{
				if (IsTargetAssociation(projectedPath))
				{
					...
					if (TargetInSourceContextRef == null)
					{
						...
						var join = new JoinTableWord(JoinKind.Left, targetCloned.SelectQuery, null, true);
						SourceContextRef.BuildContext.SelectQuery.From.Tables[0].GetJoins().Add(join);
				...
				{
					// let target context to BuildProjection
					//
		...
		class SelfTargetContext : PassThroughContext
		{
			...
			public ContextRefExpression TargetContextRef { get; }
			IClauseContext               TargetContext    => Context;
