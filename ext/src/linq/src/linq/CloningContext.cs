...
namespace mooSQL.linq.Linq
{
	...
	class CloningContext
	{
		...
		[return: NotNullIfNotNull(nameof(element))]
        Clause? CorrectRawElement(Clause? element)
		{
			if (element == null)
				return default;
				...
				if (e.NodeType == ExpressionType.Extension)
				{
					if (e is SqlPlaceholderExpression sqlPlaceholder)
					{
						return new SqlPlaceholderExpression(CorrectElement(sqlPlaceholder.SelectQuery),
							CorrectElement(sqlPlaceholder.Sql as Clause) as IExpWord, CorrectExpression(sqlPlaceholder.Path), sqlPlaceholder.ConvertType,
				...
				if (e.NodeType == ExpressionType.Extension)
				{
					if (e is SqlPlaceholderExpression sqlPlaceholder)
					{
						return new SqlPlaceholderExpression(CloneElement(sqlPlaceholder.SelectQuery),
							CloneElement(sqlPlaceholder.Sql as Clause) as IExpWord, CloneExpression(sqlPlaceholder.Path), sqlPlaceholder.ConvertType,
		...
		[return: NotNullIfNotNull(nameof(queryElement))]
        Clause? Clone(Clause? queryElement)
		{
			if (queryElement == null)
				return null;
