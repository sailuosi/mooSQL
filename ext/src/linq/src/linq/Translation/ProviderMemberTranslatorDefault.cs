...
namespace mooSQL.linq.Linq.Translation
{
	...
	public abstract class ProviderMemberTranslatorDefault : MemberTranslatorBase
	{
		...
		protected virtual Expression? ConvertToString(ITranslationContext translationContext, MethodCallExpression methodCall, TranslationFlags translationFlags)
		{
			...
			{
				toType = translationContext.CurrentColumnDescriptor.DbType;// GetDbDataType(true);
			}
			else
			{
				toType = translationContext.DBLive.dialect.mapping.GetDbDataType(typeof(string));
			}

			return translationContext.CreatePlaceholder(
				translationContext.CurrentSelectQuery, 
				new CastWord(objPlaceholder.Sql, toType, null), 
				methodCall);
		}
		...
		protected bool ProcessGetValueOrDefault(ITranslationContext translationContext, MethodCallExpression methodCall, TranslationFlags translationFlags, out Expression? translated)
		{
            ...
            IExpWord? defaultValueExpression;
			...
			{
				defaultValueExpression = new ValueWord(argumentType, translationContext.DBLive.dialect.mapping.GetDefaultValue(argumentType.SystemType.UnwrapNullable()));
			}

			var caseExpression = new ConditionWord(new IsNull(sqlExpression, true), sqlExpression, defaultValueExpression);
		...
		protected virtual IExpWord? TranslateConvertToBoolean(ITranslationContext translationContext, IExpWord sqlExpression, TranslationFlags translationFlags)
		{
			var sc = new SearchConditionWord();
			var predicate = new mooSQL.data.model.affirms.ExprExpr(sqlExpression, AffirmWord.Operator.Equal, new ValueWord(0),  translationContext.DBLive.dialect.Option.CompareNullsAsValues)
				.MakeNot();
		...
		protected virtual IExpWord? TranslateNewGuidMethod(ITranslationContext translationContext, TranslationFlags translationFlags)
		{
			return null;
		}
