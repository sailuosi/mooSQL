...
public class HHWordBag :EntityOID
{
    ...
    public int Insert() {
        ...
        var cc= kit.setTable("hh_wordbag")
            .set("HH_WordBagOID", HH_WordBagOID)
            ...
            .set("Wb_Code", Wb_Code)
            .set("Wb_Type", Wb_Type)
            .set("Wb_Remark", Wb_Remark)
            .set("Wb_Para", Wb_Para)
            .set("Wb_Parser", Wb_Parser)
            .set("Wb_Tache", Wb_Tache)
            .set("Wb_Idx", Wb_Idx)
    ...
    public int Update()
    {
        ...
        var cc = kit.setTable("hh_wordbag")
            ...
            .set("SYS_ORG", SYS_ORG)
            .set("SYS_LAST_UPD_BY", SYS_LAST_UPD_BY)
            .where("HH_WordBagOID", HH_WordBagOID)
            .doUpdate();
        return cc;
    }
    ...
    {
        if (RegxUntils.isGUID(HH_WordBagOID) == false) { 
            HH_WordBagOID= Guid.NewGuid().ToString();
            //设置插入的系统字段
        ...
        var kit = DBCash.useSQL(0);
        var cc=kit.from("hh_wordbag").where("HH_WordBagOID", HH_WordBagOID).count();
        if(cc > 0)
        {
