...
/// <summary>
/// openGauss / GaussDB 方言冒烟：默认不连真实库；设置 OPENGAUSS_CONN / GAUSSDB_CONN 时可联调。
/// </summary>
public class OpenGaussDialectSmokeTests
{
    static DBInstance DialectOnly() => DBTest.useOpenGaussDialectOnly();

    static bool TryLive(out DBInstance db)
    {
        db = null!;
        var conn = Environment.GetEnvironmentVariable("OPENGAUSS_CONN")
            ?? Environment.GetEnvironmentVariable("GAUSSDB_CONN");
        if (string.IsNullOrWhiteSpace(conn))
            return false;
        db = DBTest.useOpenGauss(conn);
        try
        {
    ...
    [Fact]
    public void Factory_Returns_OpenGaussDialect_For_OpenGauss_And_GaussDB()
    {
        var og = DialectOnly();
        og.dialect.Should().BeOfType<OpenGaussDialect>();
        og.dialect.expression.Should().BeOfType<OpenGaussExpress>();
        og.dialect.sentence.Should().BeOfType<OpenGaussSentence>();
        og.dialect.expression.paraPrefix.Should().Be(":");
        og.dialect.SupportsMerge().Should().BeTrue();

        var gb = DBTest.useGaussDBDialectOnly();
        gb.config.dbType.Should().Be(DataBaseType.GaussDB);
        gb.dialect.Should().BeOfType<OpenGaussDialect>();
        gb.dialect.SupportsMerge().Should().BeTrue();
    }

#if NET8_0_OR_GREATER
    [Fact]
    public void Net8_Connection_Is_GaussDBConnection()
    {
        using var conn = DialectOnly().dialect.getConnection();
        conn.Should().BeOfType<HuaweiCloud.GaussDB.GaussDBConnection>();
    }
#endif
    ...
    {
        var expr = (OpenGaussExpress)DialectOnly().dialect.expression;
    ...
    [Fact]
    public void Live_Crud_When_OPENGAUSS_CONN_Set()
    {
